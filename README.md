...
-   📊 **Страница статистики:** Анализируйте производительность ботов с помощью интерактивных графиков и подробных отчетов.
-   🚀 **AI-аналитика производительности**: Получайте персональные советы по улучшению ваших ботов на основе анализа данных.
-   📱 **Интеграция с Mini App:** Создавайте и подключайте веб-интерфейсы (Mini Apps) непосредственно в логике вашего бота.
-   👥 **Интерфейс для совместной работы:** Дизайн, готовый для командной работы, с возможностью делиться ботами (полная функциональность в разработке).
-   🛒 **Блоки для E-commerce:** Создавайте магазины с каталогами, корзиной и приемом платежей через Telegram Payments или Stripe (в тестовом режиме).
-   🔗 **Внешние интеграции:** Подключайте бота к любым сторонним сервисам с помощью блока "HTTP Запрос".
-   🗄️ **Встроенная база данных:** Храните информацию между сессиями с помощью блока "База данных".
...
-   ⚡ **Реалистичный симулятор:** Тестируйте своего бота в интерфейсе, который точно имитирует Telegram, включая запуск Mini Apps.
...
2.  **Создание бота:** Нажмите **"+ Создать нового бота"**. Вы можете:
    ...
    *   **Использовать шаблон:** Начните с готового шаблона, например, "Магазин" или "FAQ".
...
| Блок | Назначение |
| :--- | :--- |
| 🛍️ **Каталог товаров** | Демонстрирует список товаров (изображение, название, цена, описание) пользователю. |
| 🛒 **Корзина** | Управляет корзиной пользователя: добавляет товары, показывает содержимое или очищает. |
| 💳 **Платёж** | Инициирует процесс оплаты (интеграция с Telegram Payments). Имеет выходы "Успех" и "Ошибка". |
| 🔄 **Подписка** | Управляет подписками пользователей на услуги или контент (симуляция). |
...
| 📱 **Mini App** | Отправляет пользователю кнопку для запуска вашего веб-приложения (Telegram Mini App). |

### Блоки инструментов

| Блок | Назначение |
| :--- | :--- |
| 📍 **Запрос геолокации** | Запрашивает у пользователя его местоположение и сохраняет широту и долготу в переменные. |
| 📊 **Опрос** | Создаёт и отправляет нативный опрос Telegram с несколькими вариантами ответа. |
| 📧 **Email уведомление** | Отправляет email на указанный адрес. Поддерживает использование переменных в теме и теле письма. |
| 📈 **Интеграция с CRM** | Отправляет данные (например, имя, email, телефон) в вашу CRM для создания нового лида (симуляция). |
...
- **Интерактивная страница статистики:** Получите глубокое понимание того, как пользователи взаимодействуют с вашими ботами. Наша страница статистики включает:
    - **Ключевые метрики:** Общее количество пользователей, сообщений и средняя конверсия по всем ботам.
    - **Визуализация данных:** Интерактивные графики для отслеживания активности сообщений и распределения пользователей по ботам.
    - **AI Insights**: Персональные рекомендации от ИИ по оптимизации ваших ботов на основе данных.
    - **Детальная таблица:** Сравнение производительности каждого бота по ключевым показателям с возможностью быстрого перехода к редактору.
...
### Выполнено ✅
-   **Расширенные блоки:** Медиа, Интеграции, Инструменты.
-   **Аналитика:** Страница статистики с отслеживанием производительности.
-   **Библиотека шаблонов:** Базовый набор шаблонов для быстрого старта.
-   **Блоки E-commerce:** Каталог товаров, Корзина, Платёж, Подписка.
-   **AI-аналитика производительности**: Советы по улучшению ботов на основе данных.
-   **Улучшенный симулятор**: Интерфейс, максимально приближенный к реальному Telegram.
-   **Интеграция Mini Apps:** Добавлен блок "Mini App" для симуляции запуска веб-приложений.
-   **UI для совместной работы:** Добавлен интерфейс для демонстрации будущих возможностей коллаборации.

### В разработке и планируется 🚀

-   **Полная функциональность совместной работы:** Реализация бэкенда для синхронизации в реальном времени.
-   **Интеграция с реальными платежными шлюзами:** Подключение к API Stripe и Telegram Payments.
-   **Версионирование сценариев:** Сохранение и откат к предыдущим версиям схем бота.
-   **Расширение библиотеки шаблонов:** До 15+ готовых шаблонов для различных отраслей (e-commerce, поддержка, бронирование и т.д.).
-   **Интерактивное обучение (Improved Onboarding):** Пошаговые интерактивные уроки прямо в редакторе.
