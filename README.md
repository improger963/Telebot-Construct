...
- [🚀 Как начать работу: Пошаговое руководство](#-как-начать-работу-пошаговое-руководство)
- [🧩 Разбор основных блоков](#-разбор-основных-блоков)
...
-   📂 **Экспорт и импорт:** Легко создавайте резервные копии ваших схем в формате JSON и импортируйте их обратно в редактор.
...
1.  На панели управления нажмите кнопку **"+ Создать нового бота"**.
2.  Вам будет предложено три способа создания:

    *   **Создать с помощью AI (Рекомендуется для начала):** Выберите этот вариант, чтобы описать логику вашего бота. Например: *"Создай бота для кофейни, который спрашивает имя клиента, предлагает на выбор капучино, латте или эспрессо, а затем подтверждает заказ."* AI сгенерирует для вас стартовую схему.
    *   **Форм-бот:** Используйте этот мастер, если вам нужно собрать информацию от пользователя (например, для регистрации или опроса). Просто добавьте вопросы, которые бот должен задать.
    *   **Пользовательский бот:** Начните с чистого холста, чтобы построить уникальную логику с нуля.

3.  Введите **имя бота** и **API токен Telegram**.
    > **Совет:** Вы можете получить токен у официального бота Telegram [@BotFather](https://t.me/BotFather), отправив ему команду `/newbot`.
...
-   **Боковая панель (Sidebar) слева:**
    *   **Вкладка "Блоки":** Содержит все доступные блоки, которые можно перетащить на холст.
    *   **Вкладка "Переменные":** Отображает все переменные, которые вы используете в схеме.
...
1.  Нажмите кнопку **"Тестировать бота"** в правом верхнем углу.
...
-   Когда вы довольны результатом, нажмите **"Сохранить схему"**.
...
| Блок | Назначение | Ключевые Настройки |
| :--- | :--- | :--- |
| 🟢 **Старт** | Единственная точка входа в ваш сценарий. Каждый бот должен начинаться с этого блока. | Нет настраиваемых параметров. |
| 🔵 **Сообщение** | Отправляет текстовое сообщение пользователю. Может включать кнопки быстрого ответа, каждая из которых создаёт свою ветку в сценарии. | Текст сообщения, список кнопок. |
| 🔴 **Случайное сообщение** | Отправляет одно случайное сообщение из предопределенного списка. Полезно для разнообразия ответов. | Список текстовых сообщений. |
| 🟣 **Текстовый ввод** | Отправляет пользователю вопрос и ожидает текстового ответа. Ответ сохраняется в указанную переменную. | Текст вопроса, имя переменной. |
| ⌨️ **Ввод кнопкой** | Задаёт вопрос и предлагает пользователю выбрать один из вариантов ответа в виде кнопок. Сохраняет текст выбранной кнопки в переменную. | Текст вопроса, имя переменной, список кнопок-ответов. |
| 🟠 **Условие** | Разветвляет логику сценария. Проверяет, содержит ли переменная определённый текст. Имеет два выхода: `True` и `False`. | Имя переменной для проверки, значение для поиска. |
| 🟣 **Переключатель (Switch)** | Ветвление сценария на несколько путей на основе точного значения переменной. Мощнее, чем "Условие", для множественных проверок. | Имя переменной, список вариантов (cases) для сравнения. |
| 🖼️ **Изображение** | Отправляет изображение пользователю. | URL изображения или загрузка файла, необязательная подпись. |
| ⏳ **Задержка** | Создаёт паузу в сценарии на заданное количество секунд, имитируя "печать". | Длительность задержки в секундах. |
| 🌐 **Инлайн-клавиатура** | Отправляет сообщение с прикреплённой к нему клавиатурой. Каждая кнопка — это отдельная ветвь сценария. | Текст сообщения, список кнопок. |

---

## 🌟 Продвинутые возможности

Выведите создание ботов на новый уровень с помощью этих мощных инструментов.

### Улучшенная Панель переменных

Забудьте о гаданиях, откуда берутся данные. Наша панель переменных — это ваш центр управления данными:
- **Отслеживание:** Мгновенно видите, в каком блоке (`inputNode`) была создана переменная.
- **Поиск использования:** Просматривайте список всех блоков, которые используют конкретную переменную.
- **Отладка со статусами:** Легко выявляйте проблемы с помощью цветных меток: `DEFINED` (Определена), `UNUSED` (Не используется) и `UNDEFINED` (Используется, но не определена).

### AI-подсказки

Ускорьте свою работу с интеллектуальным помощником, встроенным прямо в холст:
- **Контекстные предложения:** Когда вы соединяете блоки, AI анализирует контекст (например, "был задан вопрос об имени") и предлагает наиболее вероятные следующие действия ("отправить приветственное сообщение с использованием имени").
- **Авто-настройка:** При выборе подсказки платформа не просто создаёт нужный блок, но и предварительно заполняет его данными, которые имеют смысл в данном контексте.
