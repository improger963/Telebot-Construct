...
const InlineKeyboardNode: React.FC<NodeProps> = ({ data, id }) => {
  const activeNodeId = useFlowStore(state => state.activeNodeId);
  const isActive = activeNodeId === id;
  const hasButtons = data.buttons && data.buttons.length > 0;
  
  return (
    <div className={`px-5 py-4 shadow-lg rounded-xl bg-surface border-t-4 border-cyan-500 w-72 ${isActive ? 'node-active-highlight' : ''}`}>
      <Handle type="target" position={Position.Top} className="w-16 !bg-accent" />
      
      <div className="flex items-center gap-3 mb-2">
        <div className="w-8 h-8 rounded-md flex items-center justify-center bg-cyan-500/20">
          <InlineKeyboardIcon className="h-5 w-5 text-cyan-500" />
        </div>
      ...
      {hasButtons ? (
        <div className="relative pt-2 border-t border-input/50 -mx-5 px-2 pb-1.5 h-10">
...
export default memo(InlineKeyboardNode);