...
const InputNode: React.FC<NodeProps> = ({ data }) => {
  return (
    <div className="px-5 py-4 shadow-lg rounded-xl bg-surface border-t-4 border-purple-500 transition-all duration-300 hover:scale-105 hover:shadow-2xl">
      <div className="flex flex-col">
        ...
        <div className="text-sm text-text-secondary max-w-xs break-words whitespace-pre-wrap">
          {data.question || 'Enter question for user...'}
        </div>
        {data.variableName && (
            <div className="text-xs text-text-secondary mt-2 pt-2 border-t border-input">
                Save response to: <span className="font-mono bg-input p-1 rounded text-purple-300">{`{${data.variableName}}`}</span>
            </div>
        )}
      </div>
